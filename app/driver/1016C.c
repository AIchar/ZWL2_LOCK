...
void ICACHE_FLASH_ATTR send_cmd(u8 mCMD, u8 data_len, u8 *data){
	...
	INFO("[INFO]1016C.send_cmd: Send CMD:%02X\r\n",mCMD);
...
char ICACHE_FLASH_ATTR dataProces(u8 *src, DataPacket* rcData){
	...
	if(*src!=0xAA || *(src+1)!=0x55){
		INFO("[ERROR]1016C.dataProces: Packet head error\r\n");
		return -1;
	}
	if (check_sum(src,CMD_PACKET_LEN)!=1)
	{
		INFO("[ERROR]1016C.dataProces: CKS ERROR\r\n");
		return -1;
	}
	rcData->CMD = src[4];
	...
	INFO("[INFO]1016C.dataProces: Recive CMD:%02X\r\n",src[4]);
	return 1;
}
	...
	if (dataProces(pData_buf,&DataPacket))
	{
		...
		if (fprint_mode == VERIFY_MODE)
		{
			...
			case CMD_GET_IMAGE:	//获取图像
				...
				{
					INFO("[ERROR]1016C.uart_rx_cb: Get Image error\r\n");
			...
			case CMD_GENERATE:	//生成特征
				...
				{
					INFO("[ERROR]1016C.uart_rx_cb: Generate error\r\n");
				}
				break;			
			case CMD_SEARCH:	//1:N验证
				...
				{
					//通过验证
					INFO("[INFO]1016C.uart_rx_cb: Finger verify pass | finger ID:%d\r\n",DataPacket.data[2]);
				...
				}else
				{
					...
					INFO("[INFO]1016C.uart_rx_cb: Finger verify fail\r\n");
			...
			case CMD_SLED_CTRL:
				break;			
			default:
				INFO("[ERROR]1016C.uart_rx_cb: Unknow error\r\n");
				break;
			}
		}
		//注册模式
		else if(fprint_mode == REGISTER_MODE)
		{
			...
			case CMD_GET_EMPTY_ID:	//获取可用ID
				if (DataPacket.dataLen==4 && DataPacket.data[0]==ERR_SUCCESS)
				{
					...
					INFO("[INFO]1016C.uart_rx_cb: Fingerprint ID: %d\r\n",fprintID);
				}else
				{
					INFO("[ERROR]1016C.uart_rx_cb: Fingerprint storage full\r\n");
					fprintID = 0;
			...
			case CMD_MERGE:	//合成模板
				if (DataPacket.dataLen==0x02 && DataPacket.data[0]==ERR_SUCCESS){
					INFO("[INFO]1016C.uart_rx_cb: Merge success...\r\n");
					sendData[0]=fprintID;
					send_cmd(CMD_STORE_CHAR,4,sendData);
				}else
				{
					INFO("[ERROR]1016C.uart_rx_cb: Merge fail\r\n");
					fprintID = 0;
					fp_verify();
				}
				break;
			case CMD_STORE_CHAR:
				if (DataPacket.dataLen==0x02 && DataPacket.data[0]==ERR_SUCCESS){
					INFO("[INFO]1016C.uart_rx_cb: Fingerprint register success | Finger ID:%d\r\n",fprintID);
...
void ICACHE_FLASH_ATTR wakeupHandle() {
	...
	{
		Key_State = 0;	//更新历史状态
		INFO("[INFO]1016C.wakeupHandle: Finger press\r\n");
		switch (fprint_mode)
		{
...
void ICACHE_FLASH_ATTR fp_register(){
	...
	INFO("[INFO]1016C.fp_register: Register mode\r\n");
}
...
void ICACHE_FLASH_ATTR fp_verify(){
	fprint_mode = VERIFY_MODE;
	INFO("[INFO]1016C.fp_verify: Verify mode\r\n");
}
...
void ICACHE_FLASH_ATTR fp_free(){
	...
	INFO("[INFO]1016C.fp_free: Free mode\r\n");
}
